  ...
  useEffect(() => {
    const fetchVariants = async () => {
      if (userRole === "Store Keeper") {
        const variantData = await getProductVariants();
        const variants = variantData.result;
        const storeProductIds = new Set(
          storeProducts.map((product) => product.productId)
        );
        const variantsForStore = variants.filter((variant) =>
          storeProductIds.has(variant.productId)
        );
        setProductVariants(variantsForStore);
      }
    };
    fetchVariants();
  }, [storeProducts]);
                ...
                {productVariants.map((variant) => (
                  <tr key={variant.productVariantId}>
                    ...
                    <td className="actions-td">
                      <AiOutlineEdit
                        onClick={() =>
                          handleEditButtonClick(variant.productVariantId)
                        }
                      />
                      <BsTrash
                        onClick={() =>
                          handleDeleteButtonClick(variant.productVariantId)
                        }
                      />
                    </td>
                  </tr>
            ...
            <div className="flex-inputs">
              <div className="form-input other-form-inputs margin">
                ...
                <input
                  ...
                  onChange={(e) => setTotalSupplyPrice(e.target.value)}
                  disabled
                />
              </div>
              <div className="form-input other-form-inputs margin">
              ...
              {userRole === "Admin" ? (
                <div className="form-input form-name-input margin">
                  <p style={{ padding: "15px" }}>
                    Please select the main Product for this Variant
                  </p>
                  <select
                    value={productId}
